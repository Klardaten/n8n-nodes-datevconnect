...
describe("AddresseeResourceHandler", () => {
  beforeEach(() => {
    ...
    fetchAddresseesSpy = spyOn(datevConnectClientModule, "fetchAddressees").mockResolvedValue([]);
    fetchAddresseeSpy = spyOn(datevConnectClientModule, "fetchAddressee").mockResolvedValue({ id: "addressee-123" });
    createAddresseeSpy = spyOn(datevConnectClientModule, "createAddressee").mockResolvedValue(undefined);
    updateAddresseeSpy = spyOn(datevConnectClientModule, "updateAddressee").mockResolvedValue(undefined);
    fetchAddresseesDeletionLogSpy = spyOn(datevConnectClientModule, "fetchAddresseesDeletionLog").mockResolvedValue([]);
  });
  ...
  describe("get operation", () => {
    test("fetches single addressee by ID", async () => {
      ...
      expect(returnData[0].json).toEqual({ id: "addressee-123", name: "Test Addressee" });
    });
  });

  describe("create operation", () => {
    test("creates new addressee", async () => {
      const mockCreatedAddressee = { id: "new-addressee", name: "New Addressee" };
      ...
      expect(returnData).toHaveLength(1);
      expect(returnData[0].json).toEqual({ id: "new-addressee", name: "New Addressee" });
    });
  });

  describe("update operation", () => {
    test("updates existing addressee", async () => {
      const mockUpdatedAddressee = { id: "addressee-123", name: "Updated Addressee" };
      ...
      expect(returnData).toHaveLength(1);
      expect(returnData[0].json).toEqual({ id: "addressee-123", name: "Updated Addressee" });
    });
  });
  ...
  describe("getDeletionLog operation", () => {
    test("fetches addressee deletion log", async () => {
      ...
      expect(returnData[1].json).toEqual({ id: "deleted-2", deletedAt: "2023-01-02" });
    });
  });
  ...
  describe("error handling", () => {
    ...
    test("handles invalid operation", async () => {
      ...
      await handler.execute("invalidOperation" as any, mockAuthContext, returnData);

      expect(returnData).toHaveLength(1);
      expect(returnData[0].json.error).toContain("not supported for resource \"addressee\"");
    });
  });
});